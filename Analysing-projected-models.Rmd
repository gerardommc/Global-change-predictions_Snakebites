...
ch.plot <- ggplot(ch.mods) + geom_raster(aes(x = x, y = y, fill = env.change)) + 
   ...
   scale_fill_gradientn(colours = jet.colours(100), 
                        ...
                        limits = c(-105, 105)) +
   labs(title = "Envenoming incidence change by 2050", x = "", y = "y (metres)", fill = "%") +
   theme_light()+
...
postscript("Plots/Total-change-2.eps", width = 9, height = 5.5)
multiplot(ch.plot, per.ch.plot.2, cols = 1,  layout = matrix(c(1, 1, 1, 1, 1, 2, 2, 2), ncol = 1))
dev.off()
```
...
proj.sn.df$scenario <- as.factor(proj.sn.df$scenario)
levels(proj.sn.df$scenario) <- c("SSP1", "SSP2\n CNRM-CM5", "SSP2\n GFDL-CM3",
                                 "SSP2\n MPI-ESM-LR", "SSP5\n CNRM-CM5",
                                 "SSP5\n GFDL-CM3", "SSP5\n MPI-ESM-LR")

proj.sn.df.1 <- subset(proj.sn.df, year %in% c(2010, 2050))
```

```{r fig.height=15, fig.width=6.5}
ggplot(proj.sn.df.1) + geom_tile(aes(x = x, y = y, fill = as.factor(Species))) + 
   facet_grid(scenario ~ year) + coord_fixed(ratio = 1) + 
   scale_fill_manual(values = rev(plasma(5))[2:5]) + 
   labs(fill = "", x = "x (metres)", y = "y (metres)") +
   theme_light() + 
   theme(legend.position = "bottom",
         axis.text.x = element_text(angle = 45, vjust = 0.5, size = 6),
         axis.text.y = element_text(size = 6),
...
pdf("Plots/Snake-species-time-scenario.pdf", width = 11, height = 7)
ggplot(proj.sn.df.1) + geom_tile(aes(x = x, y = y, fill = Species)) + 
   facet_grid(year ~ scenario) + coord_fixed(ratio = 1) + 
...
dec.maps <- ggplot(df.dec.ssp.long) + geom_raster(aes(x = x, y = y, fill = as.factor(value))) +
    ...
    labs(fill = "No. of\nRCMs", x = "x (metres)", y = "y (metres)") +
    theme_light() + 
    theme(legend.position = "right",
         axis.text.x = element_text(angle = 45, vjust = 0.7, size = 6),
         axis.text.y = element_text(size = 6),
         strip.text.x = element_text(size = 12),
         strip.text.y = element_blank(),
...
multiplot(dec.maps, dec.prop.plot, layout = matrix(c(1, 1, 1, 1, 1, 2, 2, 2), ncol = 1))
dev.off()
postscript("Plots/Decrease-maps.eps",width = 5.5, height = 6)
multiplot(dec.maps, dec.prop.plot, layout = matrix(c(1, 1, 1, 1, 1, 2, 2, 2), ncol = 1))
dev.off()
```
...
lc.2050.r <- stack(list.files("Land-cover-5km", "2050.tif", full.names = T, recursive = T))
lc.2050.df <- data.frame(rasterToPoints(lc.2050.r))
names(lc.2050.df) <- c("x", "y", scenarios)

lc.2050.mel <- reshape2::melt(lc.2050.df, id.vars = c("x", "y"))
ch.mods$land.cover <- lc.2050.mel$value

env.2050.lc <- ch.mods %>% group_by(land.cover, scenario) %>% summarise(change.land = mean(env.change, na.rm = T))
...
med.projs.dif.melt$env.change <- env.2050.lc$change.land
```

```{r fig.height=12, fig.width=5}
incid.lc.change <- ggplot(med.projs.dif.melt, aes(x = value, y = env.change,
...
```
```{r}
ggplot(med.projs.dif.melt, aes(x = value, y = env.change,
                               label = Scenario)) + geom_point(aes(colour = Scenario)) +
    facet_grid(variable~1) + 
    geom_vline(xintercept = 0, linetype = 2, colour = "grey50") + 
    geom_text(nudge_y = 2,  size = 3) +
