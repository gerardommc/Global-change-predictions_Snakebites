...
env.time <-  ggplot(med.projs.df) + 
      ...
      geom_line(aes(x = year, y = env.med, colour = scen), size = 0.25) +  
      geom_ribbon(aes(x = year, ymin = env.025, ymax = env.97, fill = scen), alpha = 0.1) +
      geom_hline(yintercept = mean(incid.data$envenomings), colour = "grey50") + 
      geom_smooth(aes(x = year, y = env.med, colour = scen, fill = scen), alpha = 0.2, size = 1, method = "lm") +
...
rates.plot <- ggplot(rates.df) + geom_boxplot(aes(x = 1, y = Estimate)) + 
   geom_point(aes(x = 1, y = Estimate, colour = proj), size = 3, position = position_dodge(0.3))+
