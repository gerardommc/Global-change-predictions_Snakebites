...
lc.urban <- reclassify(lc.r, matrix(c(4, 4, 1, -Inf, 3, 0, 4, Inf, 0), 3, 3, byrow = T))# lc.r == 4#
lc.tea <- reclassify(lc.r, matrix(c(5, 5, 1, -Inf, 4, 0), 2, 3, byrow = T))# lc.r == 2
lc.agric <- reclassify(lc.r, matrix(c(3, 3, 1, -Inf, 2, 0, 3, Inf, 0), 3, 3, byrow = T))# lc.r == 3
lc.degraded <- reclassify(lc.r, matrix(c(2, 2, 1, -Inf, 1, 0, 2, Inf, 0), 3, 3, byrow = T))# lc.r == 4
lc.forest <- reclassify(lc.r, matrix(c(1, 1, 1, -Inf, 0, 0, 1, Inf, 0), 3, 3, byrow = T))# lc.r == 5
...
ggplot(med.projs.df) + geom_point(aes(x = urban, y = env.med, colour = scenario)) +
    geom_path(aes(x = urban, y = env.med, colour = scenario),
              arrow = arrow(type = "closed", 
                          length = unit(0.1, "inches")),
              linetype = "dotted") +
    theme_light()

ggplot(med.projs.df) + geom_point(aes(x = agric, y = env.med, colour = scenario),
                                  size = 0.5) +
    geom_path(aes(x = agric, y = env.med, colour = scenario),
              arrow = arrow(type = "closed", 
                          length = unit(0.15, "inches"),
                          ends = "last"), size = 0.25) +
    theme_light()

ggplot(med.projs.df) + geom_point(aes(x = forest, y = env.med, colour = scenario),
                                  size = 0.5) +
    geom_path(aes(x = forest, y = env.med, colour = scenario),
              arrow = arrow(type = "closed", 
                          length = unit(0.15, "inches"),
                          ends = "last"), size = 0.25) +
    theme_light()
